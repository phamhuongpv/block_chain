...
export function mapAvailableMarketItems (nftContract) {
  return async (marketItem) => {
    ...
    if(metadata) return mapMarketItem(marketItem, metadata)
  }
}
...
export function mapCreatedAndOwnedTokenIdsAsMarketItems (marketplaceContract, nftContract, account) {
  return async (tokenId) => {
    ...
    console.log("mapCreatedAndOwnedTokenIdsAsMarketItems", metadata)
    ...
    if(metadata) return mapMarketItem(marketItem, metadata, tokenId, account, hasMarketApproval)
  }
}
